...
sleep(3);
system('clear');
echo "ISI KONTOL DULU LAH\n";
echo "KONTOL : ";
$coki=trim(fgets(STDIN));
system('clear');
echo "LAGI LOGIN WOY SABAR ....\n";
sleep(3);
$bet = '0.00000001';
...
$send = "play=true&coin=DOGE&client_seed=RUUFuGsc5lQPgMXdOVo&bet_amt=".$bet."&profit=".$prof."&payout=".$gp."&winning_chance=".$wcs."&prediction=2";
$log = dices($send, $coki);
...
if($login->error == 0){
	$blk = $login->balance;
	system('clear');
	echo "LOGIN BERHASIL WOY....\n";
	echo "INI SCRIPT BUAT DOGE DOANG\n";
	echo "\nPILIH WOY MAU YANG MANA\n";
...
$ga = json_decode(dices($send, $coki),true);
	...
	if($ga['win'] == 1){
		...
		echo "SENENG	|| Profit : ".$pits." DOGE	|| Ballance : ".$ga['balance']." DOGE\n";
		if($pits >= $sp){
			echo "Refresh......\n";
			if($pits >= $mjake){
	...
	if($ga['win'] == 0){
		...
		$tot = number_format($ga['balance'] - $blk, 8);
		echo "NANGIS	|| Profit : ".$pits." DOGE	|| Ballance : ".$ga['balance']." DOGE\n";
		if($pits >= $sp){
			echo "Refresh......\n";
			if($pits >= $mjake){
				echo "\nCongratulation Your Get Jakepot \nTotal Income : ".$tot."\n\n";
